...
#define GET_PARAM(name, var)                                                               \
	{                                                                                      \
		...
		{                                                                                  \
			if (0)                                                                         \
				fprintf(stderr, "warning: parameter %s not found in environment\n", name); \
		}                                                                                  \
		else                                                                               \
		{                                                                                  \
			sscanf(s, "%d", &var);                                                         \
...
int main(int argc, char *argv[])
{
	...
	if (s)
		strcpy(benchmark_name, s);
	else
		strcpy(benchmark_name, "unknown");
	...
	for (;;)
	{
		...
		for (int j = 0; j < nthreads; j++)
		{
			...
			{
				if (traces[j] && (traces[j]->cycle < traces[min_cycle_thread]->cycle))
					min_cycle_thread = j;
			}
...
void print_stats(void)
{
	...
	for (i = 0; i < ncores; i++)
		printf("core %d: %lld ", i, last_insts[i] - insts_at_warming[i]);
	printf("\nL3 misses: ");
	for (i = 0; i < ncores; i++)
		printf("core %d: %lld ", i, (l3_misses[i] - l3_misses_at_warming[i]));
	printf("\nL3 mpki: ");
	for (i = 0; i < ncores; i++)
		printf("core %d: %0.4f ", i, 1000.0 * (l3_misses[i] - l3_misses_at_warming[i]) / (double)(last_insts[i] - insts_at_warming[i]));
	printf("\n");
	if (!warming)
		for (i = 0; i < ncores; i++)
		{
